...
#see IF CAN DO THIS THE RIGHT WAY
# made a tibble where the dice was rolled for each TF to give their "lowest" letter
tf_game <- tibble(game = 1:100,
                  Alyssa = map_chr(1:100, ~roll_alpha_dice(1)),
                  Kaneesha = map_chr(1:100, ~roll_alpha_dice(1)),
                  Mitchell  = map_chr(1:100, ~roll_alpha_dice(1)),
                  June = map_chr(1:100, ~roll_alpha_dice(1))
                  )
...
tf_game %>% 
  slice(1:5) %>% 
  gt(auto_align = TRUE) %>% 
  tab_header(title = "Alpha Dice Game") %>% 
  tab_spanner(label = "TF's lowest letter per round",
              columns = vars(Alyssa,
                             Kaneesha,
                             Mitchell,
                             June
  ...
  cols_label(game = "Game Number")



# now make tibble which counts the number of vowels for Alyssa

tf_game$vowel <- 0
for (i in 1:100) {
  if (tf_game[i,2] %in% c("a", "e", "i", "o", "u")) {
    tf_game[i,6] <- 1
    } 
}
...
Alyssa had `r sum` vowels in her game of Alphah Dice.
